...
sudo npm install
sudo chmod -R 775 api/tmp
find /var/www/code/cake/app/tmp/cache/models/ -type f -exec rm '{}' \;
find /var/www/code/cake/app/tmp/cache/persistent/ -type f -exec rm '{}' \;
find /var/www/code/api/tmp/cache/models/ -type f -exec rm '{}' \;
find /var/www/code/api/tmp/cache/persistent/ -type f -exec rm '{}' \;
sudo npm install -g grunt-cli
if [[ ! -f /usr/bin/node ]]; then ln -s /usr/bin/nodejs /usr/bin/node; fi;
cd admin && sudo npm install && sudo npm run install-dnd && sudo npm run install-jgrowl && sudo composer install
cd ../
cd cake/app && sudo npm install && sudo composer install
cd ../../
cd api && sudo npm install && sudo composer install
cd ../
cd api/aws3 && sudo composer install
cd ../../
